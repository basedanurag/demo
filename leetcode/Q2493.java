...
public class Q2493 {
    ...
    //BFS function to get maximum level
    ...
    //helper function to get the maximum group size frim each connected component
    public int getMax(Map<Integer, List<Integer>> adj ,int curr , boolean[] visited, int[] levels) {
        int maxGrp = levels[curr];
        visited[curr] = true;
        for(int ngbr : adj.get(curr)) {
            if(!visited[ngbr]) {
                maxGrp = Math.max(maxGrp , getMax(adj,ngbr,visited,levels));
            }
        }
        return maxGrp;
    }
    public int magnificentSets(int n, int[][] edges) {
        ...
        for(int node = 0 ; node<n ; node++) {
            if(colors[node] == -1) {
                if(!isBipartite(mp, node, colors, 1)) {
                    return -1;
                }
            }
        }
        //bfs to find the max levels for each node 
        ...
        for(int node = 0; node < n; node++) {
            if(!visited[node]) {
                maxGroupEachComp += getMax(mp, node , visited, levels);
            }
        }
